...
def sros_check_fib_output(expected=None,timeout_s=900,interval_s=5, **router):
    with Scrapli(**router) as conn:
       conn.open()
       start_time = time.time()
       ...
       while True:
           ...
           if "Total Installed" in response.result:
              ...
              #{'NIC_Scheduler_Worker': {'key1': 1.0, 'key2': 0.08, 'key3': 0.08}}
              data = {f"fib": {'installed': float(nr_of_routes)} }
              #print(data)
              push_to_prometheus(data=data,gw="localhost:9091",jobName=router["host"])

def sros_check_fib_output6(expected=None,timeout_s=900,interval_s=5, **router):
    with Scrapli(**router) as conn:
       conn.open()
       start_time = time.time()
       #timeout_s= 900 # 15 minutes
       ...
       nr_of_routes = "0"
       # cmd="/tools dump router fib 1 summary | match Total"
       cmd="show router fib 1 summary ipv6 | match Total"
       #print("Sending \'%s\' command to %s host %s:" % (cmd,router["platform"],router["host"]))
       while True:
           response = conn.send_command(cmd)
           execution_time = time.time() - start_time
           if "Total Installed" in response.result:
              ...
              time.sleep(interval_s)
              #{'NIC_Scheduler_Worker': {'key1': 1.0, 'key2': 0.08, 'key3': 0.08}}
              data = {f"fib": {'installed_v6': float(nr_of_routes)} }
              #print(data)
              push_to_prometheus(data=data,gw="localhost:9091",jobName=router["host"])

def sros_check_fib_quasi_parallel(expected=None,timeout_s=900,interval_s=5, **router):
    conn1= Scrapli(**router)
    conn2= Scrapli(**router)
    conn1.open()
    conn2.open()
    nr_of_routes_v4 = 0
    nr_of_routes_v6 = 0
    cmd1="show router fib 1 summary | match Total"
    cmd2="show router fib 1 summary ipv6 | match Total"
       #print("Sending \'%s\' command to %s host %s:" % (cmd,router["platform"],router["host"]))
    while True:
      response1 = conn1.send_command(cmd1)
      response2 = conn2.send_command(cmd2)
      if "Total Installed" in response1.result:
         nr_of_routes_v4 = re.findall(r'\d+', response1.result)[0]
      if "Total Installed" in response2.result:
         nr_of_routes_v6 = re.findall(r'\d+', response2.result)[0]
      data = {f"fib": {'installed_v4': float(nr_of_routes_v4), 'installed_v6' : float(nr_of_routes_v6)} }
     # print(data)
      push_to_prometheus(data=data,gw="localhost:9091",jobName=router["host"])
      time.sleep(interval_s)
...
#    ## nr_of_routes = sros_check_fib_output(expected=None,timeout_s=900,interval_s=5, **target)
    sros_check_fib_quasi_parallel(expected=None,timeout_s=900,interval_s=1, **target)
